...
void Frame::AssignKPToYOLO()
{
    for (auto box : mvCones)
    {
        ...
        if(PosInGrid(x, y, lCell, uCell) && PosInGrid(x+w, y+h, rCell, dCell))
        {
            ...
            for(int i=lCell; i<=rCell;i++)
            {
                for (int j=uCell; j<=dCell;j++)
                {
                    ...
                    for(size_t r=0, rend=mCell.size(); r<rend; r++)
                    {
                        ...
                        if(kp.pt.x > x && kp.pt.x < x + w &&
                           kp.pt.y > y && kp.pt.y < y + h){
                            // cout <<"kp.pt.x "<< kp.pt.x << " x "<< x << " x + w "<< x + w << endl;
                            // cout <<"kp.pt.y "<< kp.pt.y << " y "<< y << " y + h "<< y + h << endl;
                            mvKeysCones[mCell[r]] = box.obj_id;
                        }
                    }
                }
            }
...
bool Frame::PosInGrid(double x, double y, int &posX, int &posY)
{
    ...
    posY = round((y-mnMinY)*mfGridElementHeightInv);
