...
def callBack(msg):
    ...
    yellow_points = yellow_points*normalize_factor
    blue_points = blue_points*normalize_factor

    pose2D = np.array([msg.pos_x, msg.pos_z])
    
    print('2D position ', pose2D)
    
    pose2D = pose2D*normalize_factor
    ...
    if Cones.shape[0] > 0:
        ...
        PP=PurePursuit.PPAckerman(pose2D,normal2D,mid_points,Lb=0.1,SymSteeringAngleBounds=18*3.1415/180,SphereRFactor=1,StandardDistance=1)
        print('wheels theta ',round(24-PP.SteeringAngle*180/np.pi))
        ...
        out_msg.x_cones = tmp_arr[:,0].tolist()
        out_msg.y_cones = tmp_arr[:,1].tolist()
        pub.publish(out_msg)
